     ...
     if request.method == 'POST':
        try:
            ...
            if check_if_user_exists:
                ...
                logger.info(f"User with email: {email} is authenticated successfully")
                return JsonResponse({"message": "Login successful", "responseCode": 200}, status=200)
            else:
                logger.info(f"User with email: {email} tries to login and provided Invalid username or password")
                return JsonResponse({"message": "Invalid username or password", "responseCode": 205}, status=200)            
        except Exception as ex:
            logger.error(f"Error occured while authenticating user with email {email} exception details : {ex}")
            return JsonResponse({"message": 'Error while processing', "responseCode": 400}, status=200)
